#pragma once
#pragma once
#include <iostream>
#include <mutex>
#include <cstdlib>
#include <vector>

using namespace std;

template <class T, int N>
class fineBuffer
{
public:
    fineBuffer();
    ~fineBuffer();
    bool contains(T);
    bool add(T);
    bool remove(T);
    void printList();
    void deleteList();
private:
    struct Node {
        T key;
        Node* next;
        mutex lock;
    };
    Node* head;
    Node* tail;

    int capacity;
};

/*************************************************************************
 * Initialize class variables
 * Head and tail will be used as a sentinel nodes
 * **********************************************************************/
template <class T, int N>
fineBuffer<T, N> ::fineBuffer()
{
    head = new Node;
    head->key = {};

    tail = new Node;
    tail->key = {};
    tail->next = NULL;

    head->next = tail;

    capacity = 0;
}

/*************************************************************************
 * Deallocate circular buffer memory
 * **********************************************************************/
template <class T, int N>
fineBuffer<T, N> :: ~fineBuffer()
{
    deleteList();

    delete head;
    delete tail;
}

/*************************************************************************
 * Uses fine grained locking to check if the given parameter is in
 * the circular buffer. If found, return true, else return false.
 * **********************************************************************/
template <class T, int N>
bool fineBuffer<T, N> ::contains(T key)
{
    // Acquire init(head) pred node lock
    head->lock.lock();
    Node* pred = head;

    // Acquire curr node lock
    pred->next->lock.lock();
    Node* curr = head->next;

    // While not at the of the circular buffer
    while (curr != tail) {

        // Release pred lock and update pred to curr
        pred->lock.unlock();
        pred = curr;

        // Acquire next node lock and set curr to next node
        curr->next->lock.lock();
        curr = curr->next;

        // If key was found, release pred and curr locks, and return true
        if (curr->key == key) {
            pred->lock.unlock();
            curr->lock.unlock();
            return true;
        }
    }

    // If key was not found, release pred and curr locks, and return false
    pred->lock.unlock();
    curr->lock.unlock();

    return false;
}

/*************************************************************************
 * Uses fine grained locking to attempt to add the given parameter.
 * If the parameter is already in the circular buffer, do not add again and
 * return false. If parameter is not already in the linked list, add node
 * and return true.
 * **********************************************************************/
template <class T, int N>
bool fineBuffer<T, N> ::add(T key)
{
    // Acquire init(head) pred node lock
    head->lock.lock();
    Node* pred = head;

    // Acquire curr node lock
    pred->next->lock.lock();
    Node* curr = head->next;

    // If capacity of the circular buffer is full then delete first element
    if (capacity = N) {
        head->next = head->next->next;
    }

    // While not at the end of the circular buffer
    while (curr != tail) {

        // Release pred lock and update pred to curr
        pred->lock.unlock();
        pred = curr;

        // Acquire next node lock and set curr to next node
        curr->next->lock.lock();
        curr = curr->next;

        // If key is already inserted, release pred and curr locks, and return false
        if (curr->key == key) {
            pred->lock.unlock();
            curr->lock.unlock();
            return false;
        }
    }

    // If key is not in the list, add node, release locks and return true
    Node* node = new Node;
    node->key = key;
    node->next = curr;
    pred->next = node;
    capacity++;

    pred->lock.unlock();
    curr->lock.unlock();

    return true;
}

/*************************************************************************
 * Uses fine grained locking to attempt to remove the give parameter.
 * If the parameter is not found in the circular buffer, return false. If the
 * parameter is found in the linked list, remove it and return true.
 * **********************************************************************/
template <class T, int N>
bool fineBuffer<T, N> ::remove(T key)
{
    // Acquire init(head) pred node lock
    head->lock.lock();
    Node* pred = head;

    // Acquire curr node lock
    pred->next->lock.lock();
    Node* curr = head->next;

    // While not at the of the circular buffer
    while (curr != tail) {
        // If current key is >= key, break out of traversal
        if (curr->key >= key)
            break;

        // Release pred lock and update pred to curr
        pred->lock.unlock();
        pred = curr;

        // Acquire next node lock and set curr to next node
        curr->next->lock.lock();
        curr = curr->next;
    }

    // If key was not found, release pred and curr locks, and return false
    if (curr != tail && curr->key != key) {
        pred->lock.unlock();
        curr->lock.unlock();
        return false;
    }

    // If key was found, remove it, release locks, and return true
    pred->next = curr->next;
    capacity--;

    pred->lock.unlock();
    curr->lock.unlock();

    return true;
}

/*************************************************************************
 * Display contents of circular buffer
 * **********************************************************************/
template <class T, int N>
void fineBuffer<T, N> ::printList()
{
    // Acquire head lock
    head->lock.lock();

    // Set curr to head->next since head is a sentinel node
    Node* curr = head->next;

    // Traverse linked list and display contents
    while (curr != tail) {
        cout << curr->key << " ";
        curr = curr->next;
    }

    // Release head lock
    head->lock.unlock();
}

/*************************************************************************
 * Delete contents of circular buffer
 * **********************************************************************/
template <class T, int N>
void fineBuffer<T, N> ::deleteList()
{
    Node* temp;

    while (head->next != tail) {
        temp = head->next;
        head->next = temp->next;
        delete temp;
    }

    head = new Node;
    head->key = {};

    tail = new Node;
    tail->key = {};
    tail->next = NULL;

    head->next = tail;
    capacity = 0;
}